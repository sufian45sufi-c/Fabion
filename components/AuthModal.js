...
export default function AuthModal({ isOpen, onClose, startInSignUp = false }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#050505] text-white flex overflow-hidden">
      ...
      <button
        onClick={onClose}
        className="absolute top-6 left-6 z-10 flex items-center gap-2 text-xs uppercase tracking-widest text-[#9A9A9A] hover:text-white transition-colors"
      >
        ← Back
      </button>

      {/* Left side: showcase, hidden on small screens */}
      <aside className="hidden lg:flex w-[45%] p-20 flex-col justify-center gap-6">
        <div>
          <h1
          ...
          <p className="text-[#9A9A9A] mt-3">Think deeper. Build faster. Work endlessly.</p>
        </div>

        <div className="bg-[#171717] border border-[#1f1f1f] rounded-2xl p-6 shadow-2xl max-w-[500px]">
